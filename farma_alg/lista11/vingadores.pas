...
type 
     ...
     lista = record
         ...
         vet: array [1..MAX] of conjunto; (* vetor de conjuntos *)
     end;
(* 
...
(* FUNCOES E PROCEDIMENTOS CRIADOS PARA LIDAR COM LISTAS *)
function melhor_equipe(equipes, herois: lista; hab_req: conjunto): longint;
(* 
   Retorna o indice da 'melhor' equipe, ou seja, a menor equipe que tem todas as habilidades requeridas
   Retorna 0 se nao houver nenhuma equipe com todas as habilidades requeridas
*)
var i, j, menor, melhor: longint; hab_equipe: conjunto;
begin
    menor := MAX; 
    melhor := 0;

    for i := 1 to equipes.tam do
    begin
        (* Gera um conjunto com todas as habilidades da equipe *)
        inicializar_conjunto(hab_equipe);
        for j := 1 to cardinalidade(equipes.vet[i]) do
            uniao(hab_equipe, herois.vet[equipes.vet[i][j]], hab_equipe);
  
        if contido(hab_req, hab_equipe) and (cardinalidade(equipes.vet[i]) < menor) then
        begin
            menor := cardinalidade(equipes.vet[i]);
            melhor := i;
        end;
    end;

    melhor_equipe := melhor;
end;

procedure ler_lista(var l: lista);
(* 
  Cria uma lista: um registro que contem seu tamanho (.tam) e um vetor de conjuntos (.vet)
*)
begin
    l.tam := 0;
    repeat
        l.tam := l.tam + 1;
        inicializar_conjunto(l.vet[l.tam]);
        ler_conjunto(l.vet[l.tam]);
    until eh_vazio(l.vet[l.tam]);
    l.tam := l.tam - 1;
end;

(* SOLUÇÃO PARA O PROBLEMA DOS VINGADORES *)
begin
    ler_lista(herois); (* lista de herois *)
    ler_conjunto(hab_req); (* conjunto de habilidades requeridas *)
    ler_lista(equipes); (* lista de equipes *)
    
    melhor := melhor_equipe(equipes, herois, hab_req);

    if melhor = 0 then
        writeln('NENHUMA')
    else
        imprimir_conjunto(equipes.vet[melhor]);
end.
